...
import { Accordion, AccordionItem, Button, CircularProgress, Input, Link } from "@nextui-org/react";
...
function HomePage(props) {
  ...
  const deadline = new Date('November 12, 2023 23:59:59').getTime()
  ...
  return (
    <div className="page px-3 home">
      {stateOfData === "failed" ? <CircularProgress isLoading={isLoading} color="warning" aria-label="Loading..." /> :
        <>
          <div className="w-full flex flex-row justify-between md:justify-center content-center h-20 fixed bottom-0 left-0 z-50 px-2 bg-gray-800">
            <div className="hideIt flex flex-col justify-center gap-4">
              <Timer targetDate={deadline} />
            </div>
            <div className="flex flex-row justify-center">
              <PurchaseBtn couponData={couponData} isCouponAvailable={isCouponAvailable} className="p-4 text-base m-0" style={{ padding: "4px", fontSize: "16px" }} />
            </div>
          </div>
          <div className="w-full max-w-5xl flex flex-col justify-center content-center">
            ...
            <div className="w-full flex flex-col content-center md:flex-row text-justify px-4 py-7 gap-4">
              <div className="cont1 max-w-prose">
            ...
            <div className="my-7 px-3">
              ...
              <ul className="flex flex-col gap-5">
                ...
                <li><b>Affordable Excellence:</b> Our Diwali offer brings you professional portfolios at an unbelievable price. Experience top-notch quality without burning a hole in your pocket.</li>
                ...
                <li><b>Appear in Search Results:</b> Make your portfolio strong and appear in the Web Search Results</li>
              </ul>
            </div>
            <div className="text-white w-full py-4">
              <h1 className="subtitle mb-6">FREQUENTLY ASKED QUESTIONS</h1>
              <Accordion className="text-white text-lg">
                {
                  faqs.map(prop => <AccordionItem key={prop.id} aria-label={prop.qn} title={<span className="text-white">{prop.id}. {prop.qn}</span>} className="text-white text-lg">{prop.ans}</AccordionItem>)
                }
              </Accordion>
            </div>
          </div>
